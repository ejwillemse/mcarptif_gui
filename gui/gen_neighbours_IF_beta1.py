    ...
    def removeInsertAllDoubleArcs(self, routeI, routeP = None):
        ...
        for i in range(1,nArcs-2):
            ...
            for j in insertPosition:
                ...
                neighbour = {'routes':routeP,'pos':(i,j),'arc':list(twoArcsRemoved),'costDelta':costChange1+costChange2,'loadDelta':(),'serviceDelta':(),'modifiedRoutes':modifiedRoute2,'tabus':deepcopy(tabusTemp)}
                neighbourDeltaChange = costChange1+costChange2
                self.contsructReturnAllNeighbourhoodListSingleRoute(neighbourDeltaChange, neighbour, moveType) 
                if self.stopSearch:break
    ...
    def exchangeAllDoubleArcs(self, routeI, routeP = None):
        ...
        for i in range(1,nArcs-4):
            ...
            for j in range(i+3,nArcs-2):
                ...
                neighbour = {'routes':routeP,'pos':(i,j),'arc':list(twoArcsRemoved1)+list(twoArcsRemoved2),'costDelta':costChange1+costChange2+costChange3+costChange4,'loadDelta':(),'serviceDelta':(),'modifiedRoutes':modifiedRoute2,'tabus':[i-1,i,i+1,i+2,j-1,j,j+1,j+2]}
                neighbourDeltaChange = costChange1+costChange2+costChange3+costChange4
                self.contsructReturnAllNeighbourhoodListSingleRoute(neighbourDeltaChange, neighbour, moveType) 
                if self.stopSearch:break
            ...
            for i in range(1,nArcsI-2):
                ...
                for routeJpos in nRoutesJRange:
                    ...
                    for j in range(1, nArcsJ):
                        ...
                        neighbour = {'routes':(routeIpos,routeJpos),'pos':(i,j),'arc':list(twoArcsRemoved),'costDelta':(costChangeI-serviceCostI,costChangeJ+serviceCostI),'loadDelta':(-loadI,loadI),'serviceDelta':(-serviceCostI,serviceCostI),'modifiedRoutes':(modifiedRouteI,modifiedRouteJ),'tabus':{'I':[i-1,i,i+1,i+2],'J':[j-1,j]}}
                        neighbourDeltaChange = costChangeI+costChangeJ
                        self.contsructReturnAllNeighbourhoodListMultiRoute(neighbourDeltaChange, neighbour, moveType)
                        if self.stopSearch: break
            ...
            for i in range(1,nArcsI-2):
                ...
                for routeJpos in nRoutesJRange:
                    ...
                    for j in range(1, nArcsJ-2):
                        ...
                        (costIdelta, costJdelta, loadIdelta, loadJdelta, serviceCostIdelta, serviceCostJdelta) = Deltas
                        neighbour = {'routes':(routeIpos,routeJpos),'pos':(i,j),'arc':list(twoArcsRemovedI)+list(twoArcsRemovedJ),'costDelta':(costIdelta,costJdelta),'loadDelta':(loadIdelta,loadJdelta),'serviceDelta':(serviceCostIdelta,serviceCostJdelta),'modifiedRoutes':(modifiedRouteI,modifiedRouteJ),'tabus':{'I':[i-1,i,i+1,i+2],'J':[j-1,j,j+1,j+2]}}
                        neighbourDeltaChange = costIdelta+costJdelta
                        self.contsructReturnAllNeighbourhoodListMultiRoute(neighbourDeltaChange, neighbour, moveType)
                        if self.stopSearch: break
...
class genAllNeighbourhoods(SingleRouteRemoveInsertProcedure, MultipleRouteRemoveInsertProcedure):
    ...
    def genFirstNeighbourOrdered(self, routes, loads, routeIndex, costIndex):
        ...
        if not self.stopSearch:
            self.generateAllRoutesNeighbourhoodsSingleRoutesOrder(routes, 'RemoveInsert') #2
        if not self.stopSearch:
            self.generateAllRoutesNeighbourhoodsSingleRoutesOrder(routes, 'TwoExchange') #3
        if not self.stopSearch:
            self.generateAllNeighbourhoodsMulitRoutesOrdered(routes, loads, 'TwoRemoveInsert') #4
        if not self.stopSearch:
            self.generateAllRoutesNeighbourhoodsSingleRoutesOrder(routes,'TwoRemoveInsert') #5
        if not self.stopSearch:
            self.generateAllRoutesNeighbourhoodsSingleRoutesOrder(routes, 'Exchange') #6
        if not self.stopSearch:
            self.generateAllNeighbourhoodsMulitRoutesOrdered(routes, loads, 'Exchange') #7
        if not self.stopSearch:
            self.generateAllNeighbourhoodsMulitRoutesOrdered(routes, loads, 'TwoExchange') #8
        return(self.bestNeighbour)
